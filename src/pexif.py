...
# Example:
#
# jpeg = pexif.JpegFile.fromFile("bar.jpg")
# exif = jpeg.get_exif()
# ....
...
class DefaultSegment:
    ...
    def dump(self, fd):
        ...
        extra information."""
        fd.write(" Section: [%5s] Size: %6d" % (jpeg_markers[self.marker][0], len(self.data)))
...
class StartOfScanSegment(DefaultSegment):
    ...
    def dump(self, fd):
        ...
        fd.write(" Section: [  SOS] Size: %6d Image data size: %6d" % (len(self.data), len(self.img_data)))
...
class IfdData():
    ...
    def extra_ifd_data(self, _offset):
    ...
    def __init__(self, e, offset, exif_file, mode, data=None):
        ...
        next_offset = unpack(e + "I", data[offset + 2 + 12 * num_entries: offset + 2 + 12 * num_entries + 4])[0]
        debug("OFFSET %s - %s" % (offset, next_offset))
        ...
        for i in range(num_entries):
            ...
            debug("%s %s %s %s %s" % (hex(tag), exif_type, exif_type_size(exif_type), components, the_data))
            ...
            if tag in self.embedded_tags:
                ...
                if exif_type == BYTE or exif_type == UNDEFINED:
                    ...
                    for j in range(components):
                        actual_data.append(Rational(*unpack(e + t, the_data[j * 8: j * 8 + 8])))
                else:
                    print("raise 'Cannot handle this'")
            ...
            debug("%-40s %-10s %6d %s" % (self.tags.get(tag, (hex(tag), 0))[0], ExifType.lookup[exif_type], components, actual_data))
    ...
    def getdata(self, e, offset, last=0):
        ...
        for tag, exif_type, the_data in self.entries:
            ...
            out_entries.append((tag, magic_type, magic_components, actual_data))
    ...
    def dump(self, fd, indent=""):
        """Dump the IFD file"""
        fd.write(indent + "<--- %s start --->" % self.name)
        for entry in self.entries:
            ...
            if (self.isifd(data)):
                ...
                if data and len(data) == 1:
                    data = data[0]
                fd.write(indent + "  %-40s %s" % (self.tags.get(tag, (hex(tag), 0))[0], data))
        fd.write(indent + "<--- %s end --->" % self.name)
...
def ifd_maker_note(e, offset, exif_file, mode, data):
    ...
    if exif_file.make == "Canon":
        ...
        if header != "FUJIFILM":
            raise JpegFile.InvalidFile("This is FujiFilm JPEG. Expecting a makernote header <FUJIFILM>. Got <%s>." % header)
        ...
        if unknown_maker_note_as_error:
            msg = "Unknown maker: %s. Can't currently handle this." % exif_file.make
...
class IfdThumbnail(IfdTIFF):
    ...
    def ifd_handler(self, data):
        size = None
        offset = None
        for (tag, _exif_type, val) in self.entries:
            if (tag == 0x201):
                offset = val[0]
            if (tag == 0x202):
                size = val[0]
        if size is None or offset is None:
            raise JpegFile.InvalidFile("Thumbnail doesn't have an offset and/or size")
        object.__setattr__(self, 'jpeg_data', data[offset:offset + size])
        if len(self.jpeg_data) != size:
            raise JpegFile.InvalidFile("Not enough data for JPEG thumbnail. Wanted: %d got %d" % (size, len(self.jpeg_data)))
...
class ExifSegment(DefaultSegment):
    ...
    def parse_data(self, data):
        ...
        if (exif != "Exif"):
            raise self.InvalidSegment("Bad Exif Marker. Got <%s>, expecting <Exif>" % exif)
        ...
        if self.tiff_endian == "II":
            ...
            raise JpegFile.InvalidFile("Bad TIFF endian header. Got <%s>, expecting <II> or <MM>" % self.tiff_endian)
    ...
    def dump(self, fd):
        fd.write(" Section: [EXIF] Size: %6d" % (len(self.data)))
...
class JpegFile:
    ...
    @staticmethod
    def fromString(astr, mode="rw"):
        """Return a new JpegFile object taking data from a string."""
        return JpegFile(StringIO(astr), "from buffer", mode=mode)
    ...
    def __init__(self, aninput, filename=None, mode="rw"):
        ...
        soi_marker = aninput.read(len(SOI_MARKER))

        # The very first thing should be a start of image marker
        if (soi_marker != SOI_MARKER):
            raise self.InvalidFile("Error reading soi_marker. Got <%s> should be <%s>" % (soi_marker, SOI_MARKER))
        ...
        while True:
            ...
            if (delim != DELIM):
                raise self.InvalidFile("Error, expecting delimiter. Got <%s> should be <%s>" % (delim, DELIM))
            if mark == EOI:
                # Hit end of image marker, game-over!
                break
            head2 = aninput.read(2)
            size = unpack(">H", head2)[0]
            data = aninput.read(size - 2)
            ...
            for segment_class in possible_segment_classes:
                try:
                    ...
                    attempt = segment_class(mark, aninput, data, self.mode)
    ...
    def dump(self, fd=sys.stdout):
        ...
        fd.write("<Dump of JPEG %s>" % self.filename)
        for segment in self._segments:
            segment.dump(fd)
    ...
    def remove_metadata(self, paranoid=True):
        ...
        if paranoid:
            self._segments = [seg for seg in self._segments if seg.code in paranoid_keep_list]
        else:
            self._segments = [seg for seg in self._segments if not (seg.code == 'COM' or seg.code.startswith('APP'))]
    ...
    def get_geo(self):
        ...
        def convert(x):
            ...
            return (float(deg.num) / deg.den) +  \
                (1 / 60.0 * float(mins.num) / mins.den) + \
    ...
    def set_geo(self, lat, lng):
        ...
        sign, deg, mins, sec = JpegFile._parse(lat)
        ref = "N"
        if sign < 0:
            ref = "S"

        gps.GPSLatitudeRef = ref
        gps.GPSLatitude = [Rational(deg, 1), Rational(mins, 1), Rational(sec, JpegFile.SEC_DEN)]

        sign, deg, mins, sec = JpegFile._parse(lng)
        ref = "E"
        if sign < 0:
            ref = "W"
        gps.GPSLongitudeRef = ref
        gps.GPSLongitude = [Rational(deg, 1), Rational(mins, 1), Rational(sec, JpegFile.SEC_DEN)]
