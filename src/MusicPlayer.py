...
class Song():
	def __init__(self, text="", mode=0, _id=-1, navigator=False, artistID=0, albumID=0, title="", artist="", filename="", bitrate=None,
		length="", genre="", track="", date="", album="", playlistID=0, genreID=0, songID=0, join=True, PTS=None, isDVB=False):
		self.text = text
		self.mode = mode
...
class MDCMusicPlayer(Screen, InfoBarBase, InfoBarSeek, InfoBarNotifications):
	...
	def __init__(self, session, file_list, file_index):
		print("MDC: MusicPlayer: MDCMusicPlayer: __init__: file_index: %s" % file_index)
		...
		self.GoogleCoverPathChange()
		self.gaplessConfigChange()
		...
		self.googleimage = config.plugins.mediacockpit.googleimagepath.value
	...
	def startRun(self):
		...
		for x in self.file_list:
			...
			if x[FILE_MEDIA] == "music":
				self.song_list.append((Song(text=os.path.basename(filename), filename=filename),))
				if filename == self.file_list[self.file_index][FILE_PATH]:
					self.song_index = i
				i += 1
		self.original_song_list = self.song_list[:]
		#print("MDC: MusicPlayer: MDCMusicPlayer: startRun: song_index: %s, song_list: %s" % (self.song_index, str(self.song_list)))

	def alsasinkConfigChange(self):
		eMerlinMusicPlayer.getInstance().enableAlsa(config.plugins.mediacockpit.alsasink.value)

	def gaplessConfigChange(self):
	...
	def GoogleCoverPathChange(self):
		self.googleimage = config.plugins.mediacockpit.googleimagepath.value
		self.googleDownloadDir = os.path.join(config.plugins.mediacockpit.googleimagepath.value, "downloaded_covers/")
	...
	def getNextFile(self, _arg):
		...
		filename = self.song_list[self.song_index][0].filename
	...
	def __onShown(self):
		if self.first_start:
			...
			self.setCovers("")
			if self.song_list:
				self.playSong(self.song_list[self.song_index][0].filename)
		else:
			if self.current_title is not None:
				self.summaries.setText(self.current_title)

	def exit(self, file_index=-1):
		eMerlinMusicPlayer.getInstance().setFunc(None)
		if file_index == -1:
			file_index = 0
			for i, x in enumerate(self.file_list):
				if x[FILE_PATH] == self.song_list[self.song_index][0].filename:
					file_index = i
					break
		self.close(file_index)

	def playSong(self, filename):
		print("MDC-I: MusicPlayer: MDCMusicPlayer: playSong: filename: %s" % filename)
		self.service_started = False
		...
		print("MDC-I: MusicPlayer: MDCMusicPlayer: playSong: sref: %s" % sref.toString())

		if self.song_list[self.song_index][0].isDVB:
			sref.setName(self.song_list[self.song_index][0].text)
	...
	def __buffering(self):
		...
		if current_service is not None:
			streamed = current_service.streamed()
			bufferInfo = streamed.getBufferCharge()
			percent = bufferInfo[0]
			text = ""
			if percent != 100:
				text = _("Buffering") + ": %d%% ..." % percent
	...
	def updateMusicInformation(self, artist="", title="", album="", genre="", year="", track="", clear=False):
		...
		if self.song_list[self.song_index][0].filename.startswith("http://") and album_year == "":
			album_year = self.song_list[self.song_index][0].text
		...
		# for lyrics
		self.song_list[self.song_index][0].title = title
		self.song_list[self.song_index][0].artist = artist
		self.summaries.setText(title)

		if not self.got_embedded_cover_art:
			if not self.setFolderCover():
				self.updateCover(artist, album)

	def setFolderCover(self):
		path = os.path.dirname(os.path.abspath(self.current_filename))
	...
	def getGoogleCover(self, artist, album):
		...
		if (artist != "" and album != "") or ("://" in self.current_filename and self.sTitle != ""):
			if (artist != "" and album != ""):
	...
	def playPrevious(self):
		if not self.repeat:
			self.song_index = self.song_index - 1 if self.song_index else len(self.song_list) - 1
		self.playSong(self.song_list[self.song_index][0].filename)

	def getNextTitle(self):
		index = self.song_index
		if not self.repeat:
			index = (self.song_index + 1) % len(self.song_list)

		audio, _is_audio, title, _genre, artist, _album, _tracknr, _track, _date, _length, _bitrate = getID3Tags(self.song_list[index][0].filename)
		text = title
		if audio and artist:
	...
	def shuffleList(self):
		...
		if self.shuffle:
			...
			self.song_list = self.original_song_list[:]
			self["shuffle"].hide()

		for i, x in enumerate(self.song_list):
			if x[0].filename == self.current_filename:
				self.song_index = i
				break
		self["nextTitle"].setText(self.getNextTitle())
	...
	def showLyrics(self):
		self.session.open(Lyrics, self.song_list[self.song_index][0])
