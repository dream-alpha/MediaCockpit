...
class MediaCockpit(Screen, Tiles, Menu, HelpableScreen):
	...
	def readFileList(self, load_dir, load_path=""):
		...
		if load_dir == "":
			...
			if load_dir == "PLAYLIST" and load_path:
				...
				self.file_list += sortList(FileManager.getInstance(ID).getFileListByList(playlist))
			else:
				if self.show_dirup_tile:
					self.file_list.append(createParentDirEntry(load_dir))
				self.file_list += sortList(FileManager.getInstance(ID).getDirList(load_dir))
				self.file_list += sortList(FileManager.getInstance(ID).getFileList(load_dir))
	...
	def startSlideshow(self):
		...
		if self.media_list and self.file_list[self.file_index][MDC_IDX_MEDIA] in [MDC_MEDIA_TYPE_PICTURE, MDC_MEDIA_TYPE_MOVIE]:
			...
			self.session.openWithCallback(self.SlideshowCallback, Slideshow, self, self.media_list, self.media_index, self.slideshow, self.song_list)
		elif self.song_list:
			self.song_index = getIndex(self.song_list, start_path)
			self.session.openWithCallback(self.CockpitMusicPlayerCallback, CockpitMusicPlayer, self.song_list, self.song_index, self.service_center)
		...
		if self.file_list:
			...
			if afile[MDC_IDX_MEDIA] == MDC_MEDIA_TYPE_UP:
				...
				if afile[MDC_IDX_MEDIA] == MDC_MEDIA_TYPE_PICTURE:
					...
					self.session.openWithCallback(self.SlideshowCallback, Slideshow, self, self.media_list, media_index, False)
	...
	def openInfo(self):
		if self.file_list:
			afile = self.file_list[self.file_index]
			if afile[MDC_IDX_MEDIA] in MDC_MEDIA_TYPE_FILE:
				self.session.openWithCallback(self.setTilesCursor, MediaInfo, self, self.file_list, self.file_index)
	...
	def loadCacheDirResponse(self, response):
		if response:
			FileManager.getInstance(ID).loadDatabaseDir(getDir(self.file_list, self.file_index))
			self.session.open(FileManagerProgress, FILE_OP_LOAD)
			self.return_dir = getDir(self.file_list, self.file_index)
			self.return_path = getPath(self.file_list, self.file_index)
			logger.debug("return_dir: %s, return_path: %s", self.return_dir, self.return_path)
	...
	def loadCacheBookmarkResponse(self, response):
		if response:
			FileManager.getInstance(ID).loadDatabaseDir(getFile(self.file_list, self.file_index)[MDC_IDX_BOOKMARK], True)
			self.session.open(FileManagerProgress, FILE_OP_LOAD)
			self.return_dir = getFile(self.file_list, self.file_index)[MDC_IDX_BOOKMARK]
			self.return_path = ""
			logger.debug("return_dir: %s, return_path: %s", self.return_dir, self.return_path)
